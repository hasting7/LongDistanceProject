...
static void epd_send_data_byte(uint8_t data) {
    gpio_set_level(EINK_PIN_DC, 1); // data
    spi_transaction_t t = {
        .length = 8,
        .tx_buffer = &data,
    };
...
static void epd_reset(void) {
    ...
    gpio_set_level(EINK_PIN_RST, 1);
    vTaskDelay(pdMS_TO_TICKS(20));
    ESP_LOGI(TAG, "epd_reset: RST pulsed, BUSY now %d", gpio_get_level(EINK_PIN_BUSY));
}
...
void eink_init(void) {
    ...
    spi_bus_config_t buscfg = {
        ...
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
        .max_transfer_sz = EINK_BUFFER_SIZE + 8,
    };
...
static bool epd_wake_and_configure(void) {
    ...
    epd_send_data_byte(0x00);
    epd_send_data_byte(EINK_WIDTH_BYTES - 1);

    epd_send_cmd(0x45); // set RAM Y start/end (0..295)
    epd_send_data_byte(0x00);
    epd_send_data_byte(0x00);
    epd_send_data_byte((EINK_HEIGHT - 1) & 0xFF);
    epd_send_data_byte((EINK_HEIGHT - 1) >> 8);
...
bool display_screen(uint8_t *frame_ptr) {
    ...
    for (int i = 0; i < EINK_BUFFER_SIZE; i++) {
        frame_ptr[i] ^= 0xff;
    }
#endif
    ...
    epd_send_cmd(0x24);
    epd_send_data_buf(frame_ptr, EINK_BUFFER_SIZE);
